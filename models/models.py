...
def conv3x3_bn_relu(in_planes, out_planes, stride=1):
	return nn.Sequential(conv3x3(in_planes, out_planes, stride), SynchronizedBatchNorm2d(out_planes),
...
class ModelBuilder():
	...
	def weights_init(self, m):
		...
		if classname.find('Conv') != -1:
			...
			m.bias.data.fill_(1e-4)  # elif classname.find('Linear') != -1:  #    m.weight.data.normal_(0.0, 0.0001)
	...
	def build_encoder(self, arch='resnet50_dilated8', fc_dim=512, weights=''):
		...
		if arch == 'resnet18':
			...
			from .MobileNets import mobilenet_v2
			orig_mobilenet = mobilenet_v2(pretrained=pretrained)
			net_encoder = MobilveNetv2(orig_mobilenet)
		else:
...
class MobilveNetv2(nn.Module):
	def __init__(self, ori_mobilenet):
		super(MobilveNetv2, self).__init__()
...
class PPMBilinear(nn.Module):
	def __init__(self, num_class=150, fc_dim=4096, use_softmax=False, pool_scales=(1, 2, 3, 6)):
		...
		for scale in pool_scales:
			self.ppm.append(
				nn.Sequential(nn.AdaptiveAvgPool2d(scale), nn.Conv2d(fc_dim, 512, kernel_size=1, bias=False),
		...
		self.conv_last = nn.Sequential(
			...
			SynchronizedBatchNorm2d(512), nn.ReLU(inplace=True), nn.Dropout2d(0.1),
...
class PPMBilinearDeepsup(nn.Module):
	...
	def forward(self, conv_out, segSize=None):
		...
		for pool_scale in self.ppm:
			ppm_out.append(
				nn.functional.upsample(pool_scale(conv5), (input_size[2], input_size[3]), mode='bilinear', align_corners=False)
			)
		ppm_out = torch.cat(ppm_out, 1)
...
class UPerNet(nn.Module):
	def __init__(self, num_class=150, fc_dim=4096, use_softmax=False, pool_scales=(1, 2, 3, 6),
		...
		for fpn_inplane in fpn_inplanes[:-1]:  # skip the top layer
			self.fpn_in.append(
				nn.Sequential(nn.Conv2d(fpn_inplane, fpn_dim, kernel_size=1, bias=False),
	...
	def forward(self, conv_out, segSize=None):
		...
		for pool_scale, pool_conv in zip(self.ppm_pooling, self.ppm_conv):
			ppm_out.append(pool_conv(
				nn.functional.upsample(pool_scale(conv5), (input_size[2], input_size[3]), mode='bilinear',
					align_corners=False)))
		ppm_out = torch.cat(ppm_out, 1)
		...
		for i in reversed(range(len(conv_out) - 1)):
			...
			f = nn.functional.upsample(f, size=conv_x.size()[2:], mode='bilinear',
